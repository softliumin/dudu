...
function showOkMsg(msg){
    layer.open({content: msg,time:2,type:2});
}
// 保存查询的用户
$(function(){
    $('.query_btn').click(function(){
        var usernameCookie = $.trim($('input[name="username"]').val())
        if(usernameCookie && usernameCookie.length == 11){
            setCookies("username",usernameCookie);
        }
    })
})